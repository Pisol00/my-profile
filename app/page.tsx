...
import ClearerBgSection from "@/components/ClearerBgSection";
...
export default function ProfilePage() {
  ...
  const blurredBackground = "bg-gradient-to-b from-background to-background/50";
  ...
  blurAmount="8px"         // ลดความเบลอลงจาก 8px เป็น 3px
  opacity={0.8}            // แก้ไขจาก 8 เป็น 0.8 (ค่าต้องอยู่ระหว่าง 0-1)
  className="min-h-screen flex items-center pt-16 bg-gradient-to-b from-transparent to-background/20"  // เพิ่ม gradient
...
>
  <div className="container mx-auto max-w-7xl px-4 py-24">
    <div className="flex flex-col md:flex-row items-center justify-between gap-12">
      ...
      >
        {/* Badge ที่ปรับปรุงให้ดูโดดเด่น */}
        <span className="inline-block px-4 py-1.5 rounded-full bg-primary/20 text-primary text-sm font-semibold mb-6 shadow-sm border border-primary/10 backdrop-blur-sm">
          {currentLang === "en"
            ? "Full-Stack Developer"
            : "นักพัฒนาเว็บแบบ Full-Stack"}
        </span>
        
        {/* ชื่อที่ปรับแต่งให้โดดเด่น */}
        <h1 className="text-4xl md:text-6xl font-bold text-foreground mb-4 drop-shadow-sm">
          {profile.name}
        </h1>
        
        {/* Title ที่มี gradient text สวยงาม */}
        <p className="text-xl md:text-2xl mb-6 gradient-text font-semibold">
          {profile.title}
        </p>
        
        {/* Bio ที่ดูอ่านง่ายด้วย glass effect */}
        <p className="text-lg text-foreground mb-8 max-w-2xl bg-background/30 backdrop-blur-sm p-5 rounded-lg shadow-sm border border-white/10">
          {profile.bio}
        </p>

        {/* ปุ่มที่ปรับให้ดูทันสมัยขึ้น */}
        <div className="flex flex-wrap gap-4 justify-center md:justify-start">
          <Button
            asChild
            size="lg"
            className="gap-2 gradient-button shadow-md button-hover px-6 rounded-lg"
          >
          ...
          <Button
            ...
            asChild
            size="lg"
            className="gap-2 button-hover shadow-md bg-background/40 backdrop-blur-sm hover:bg-background/60 border-white/20 px-6 rounded-lg"
          >
            <a
        ...
        {/* ข้อมูลติดต่อที่ดูโมเดิร์นขึ้น */}
        <div className="flex flex-wrap gap-4 mt-8 justify-center md:justify-start">
          <div className="flex items-center gap-2 bg-background/30 backdrop-blur-sm p-2 px-4 rounded-full shadow-sm border border-white/10">
      ...
      <AnimatedSection
        ...
        disabled={!animationsEnabled}
      >
        {/* รูปโปรไฟล์ที่ปรับให้มี glow effect และดูโดดเด่น */}
        <div
          ...
          style={{
            boxShadow: '0 0 30px rgba(var(--primary-rgb, 100, 100, 255), 0.2)',
          }}
        >
          <Image
            src={profile.avatar}
    ...
    {/* ปุ่มเลื่อนลงที่ปรับให้ดูโดดเด่นขึ้น */}
    <div className="flex justify-center mt-16">
      <Button
        variant="ghost"
        size="lg"
        onClick={() => scrollToSection(skillsRef)}
        className="text-primary bg-background/20 backdrop-blur-sm hover:bg-background/40 rounded-full p-2 transition-all hover:shadow-md hover:scale-110"
      >
        <ChevronDown size={28} />
      </Button>
    </div>
                ...
                <CardContent className="p-8">
                  ...
                  <div className="mt-8">
                    <Button
                      asChild
                      size="lg"
                      className="w-full gradient-button button-hover shadow-custom"
                    >
                      <a href={`mailto:${profile.email}`}>
                        {currentLang === "en"
                          ? "Send me a message"
